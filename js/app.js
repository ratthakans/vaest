/* ═══ helpers ═══ */
  ...
  function renderRailUsage(){
    ...
    if(q.internal){ // team account — no meter to show; a quiet status chip instead of a blank gap
      ...
      el.innerHTML='<span class="qn"><span>Team · unlimited</span><em>v'+VERSION+'</em></span>';
  ...
  async function streamAPI(mode,messages,system,onText){
    ...
    try{for(;;){const {done,value}=await reader.read();if(done)break;
      ...
      // A dropped connection used to throw away everything that had already arrived. The server
      // has finished and billed by then, so the user paid for a document they never saw — and on
      // a phone (tunnel, lift, wifi handover) this is not rare. Pressing Stop already keeps the
  ...
  function buildDocHTML(forPrint){
    ...
    const CSS=(forPrint?'@page{size:A4 portrait;margin:20mm 0}':'')
      ...
      +'body{font-family:"Newsreader","Noto Serif Thai",Georgia,serif;color:#26251f;background:#fdfdf9;line-height:1.85;font-size:'+(forPrint?'11.5pt':'17px')+'}'
      ...
      +'h2{font-family:"Newsreader","Noto Serif Thai",Georgia,serif;font-size:'+(forPrint?'16pt':'25px')+';margin:6px 0 12px;letter-spacing:-.005em;font-weight:500;color:#1a1a18}'
      +'.c p{margin:0 0 12px}.c strong{font-weight:600;color:#0e0e0e}'
      +'.c .ph{font-family:"Newsreader","Noto Serif Thai",Georgia,serif;font-weight:500;color:#1a1a18;margin:16px 0 7px;font-size:'+(forPrint?'12.5pt':'17px')+'}'
      ...
      +'th{font-family:"Newsreader","Noto Serif Thai",Georgia,serif;text-align:left;font-weight:500;border-bottom:1.5px solid rgba(27,24,21,.16);padding:8px 16px 8px 0;font-size:'+(forPrint?'9pt':'13.5px')+'}'
  ...
  function buildDeckHTML(slides,title){
    ...
    return '<!doctype html><html lang="'+docLang(title+' '+JSON.stringify(slides))+'"><head><meta charset="utf-8"><title>'+esc2(title)+' — VÆST</title>'
      ...
      +'.sl li::before{content:"";position:absolute;left:0;top:.62em;width:8px;height:2px;border-radius:2px;background:linear-gradient(90deg,#4fc3ff,#eb5939)}'
      ...
      +'.rule{width:56px;height:3px;border-radius:3px;background:linear-gradient(90deg,#4fc3ff,#dff4ff,#f08a5e,#eb5939);margin:22px 0}'
