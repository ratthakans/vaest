...
export const PLANS = {
  basic:     { docs: 20,  week: 8,  refine: false },
  pro:       { docs: 60,  week: 20, refine: true  },
  director:  { docs: 120, week: 40, refine: true  },
  unlimited: { docs: Infinity, week: Infinity, refine: true },
};
...
// Check the monthly + weekly document quota and, if under, atomically bump the counters.
// Returns {ok:true} or {ok:false, scope:'week'|'month', cap}. Caller must fail-open on throw.
export async function checkAndBumpDoc(email, plan) {
  ...
  await writeUsageRow(email, { ...d, docMonth: month, docCount: dMonth + 1, docWeek: wk, docWeekCount: dWeek + 1 });
  return { ok: true };
}
