...
// Pure: credit one boost onto a usage-data object. Idempotent per Stripe session id —
// both /api/confirm and the webhook may try to apply the same purchase.
export function applyBoost(d, sid) {
  ...
  return {
    ...d, extraMonth: month,
    extraDocs: ex.docs + BOOST.docs, extraRefines: ex.refines + BOOST.refines,
