...
// Which database this deployment talks to. Hardcoding it meant every deployment — production,
// every Vercel preview, anything anyone ran locally — wrote to the same rows as live customers,
// which is the single code-level reason there is no staging: you cannot stand up a second
// environment when the first one's address is compiled into it.
//
// The defaults are the production project, so nothing changes without env vars being set. A
// staging deployment sets these two on Preview scope and is instantly talking to its own data.
// The key is the publishable one — it ships in the client bundle already and is not a secret.
export const SB = {
...
// Buckets are named after ENGINES, never after models — the bucket travels to the browser in the
// [[USAGE]] trailer, so a bucket called `sonnet` was a model id shipped to every reader. `galdr`
// used to hold the Haiku rate while Galdr itself billed under `sonnet`, which meant the one name
// that looked self-explanatory was the one that was wrong: an edit trusting it would have
// under-billed Galdr threefold and quietly broken the 30% floor.
export const RATES = {
  odin:         { in: 180, out: 900  },   // Odin      $5/$25 @ ฿36
  norrsken:     { in: 360, out: 1800 },   // Norrsken  $10/$50
  galdr:        { in: 108, out: 540  },   // Galdr     $3/$15
  'galdr-lite': { in: 36,  out: 180  },   // the cheap fallback behind Galdr, and the topic label
};
...
//
// A pack buys two things: documents/Refines, which never expire, and the spend headroom to use
// them — which did. The headroom was keyed on packMonth, so a pack bought on the 31st extended the
// ceiling for one day and then evaporated while the documents it paid for remained. On the 1st the
// customer held 15 unexpired credit documents, hit the plan's own ceiling, and was told to "add a
// usage credit pack". They already had.
//
// The headroom is a balance now, not a monthly flag: it persists exactly as long as the credit
// does, and is drawn down only by spend that lands ABOVE the plan's own ceiling — so a customer
// who never exceeds their plan keeps the headroom for the month they need it. Law #5 is unchanged:
// every baht of headroom was paid for at the same 70%-of-net basis as the plan itself.
export function applySpend(d, thb, baseCap) {
  const month = monthKey();
  const cur = d.spendMonth === month ? (d.spendTHB || 0) : 0;
  const next = +(cur + thb).toFixed(4);
  let creditSpend = creditSpendOf(d);
  if (Number.isFinite(baseCap) && creditSpend > 0) {
    const over = Math.max(0, next - Math.max(cur, baseCap));   // only the part above the plan's own cap
    if (over > 0) creditSpend = Math.max(0, +(creditSpend - over).toFixed(4));
  }
  return { ...d, spendMonth: month, spendTHB: next, creditSpend };
}
// Rows written before the balance existed recorded packs bought this month as packMonth/packCount.
// Read those as the headroom they paid for rather than stranding it.
export function creditSpendOf(d) {
  if (typeof d.creditSpend === 'number') return d.creditSpend;
  const packs = d.packMonth === monthKey() ? (d.packCount || 0) : 0;
  return packs * BOOST_SPEND;
}
// The month's spend ceiling: the plan's own cap plus whatever pack headroom is still unspent.
export function spendCapFor(plan, d) {
  const base = plan && Number.isFinite(plan.spendCap) ? plan.spendCap : Infinity;
  if (!Number.isFinite(base)) return Infinity;
  return base + creditSpendOf(d);
}
...
export function applyBoost(d, sid, packs = 1) {
  ...
  return {
    ...
    creditSpend: +(creditSpendOf(d) + BOOST_SPEND * n).toFixed(4), // the headroom to actually use them
...
// readRow returns null both for "no row yet" and for "the read failed", and every caller wrote
// `|| {}`. So a Supabase outage made every counter read zero AT ONCE — tokens, documents, Refines
// and real baht — and each gate independently concluded there was room. The one system designed to
// bound cost failed open in all four dimensions simultaneously, for as long as the outage lasted.
//
// A new account and an unreachable database are not the same fact and must stop being the same
// value. `_unreadable` marks the second; callers that spend real money check it (see api/chat.js).
export async function readUsageData(email) {
  const row = await readRow(usageRow(email));
  if (row === null) {
    // distinguish the two: a genuine miss returns an empty row from PostgREST, a failure throws or
    // returns non-ok — readRow collapses both to null, so re-ask once with the outcome exposed.
    try {
      const r = await sbFetch(`/rest/v1/vaest_state?email=eq.${encodeURIComponent(usageRow(email))}&select=data`, { headers: svcHeaders });
      if (!r.ok) return { _unreadable: true };
      const rows = await r.json();
      return (rows[0] && rows[0].data) || {};
    } catch (e) { return { _unreadable: true }; }
  }
  return row;
}
