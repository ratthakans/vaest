...
export function isAllowed(email) {
  ...
  return isInternal(e) || INVITED.has(e) || Object.prototype.hasOwnProperty.call(PLAN_MAP, e);
}
...
// spendCap = the hard COST ceiling per month, ฿ — set at 70% of the plan price so the
// worst-case margin can never drop below 30% regardless of which engines a customer
// leans on (doc/token caps approximate behaviour; this one measures actual baht).
// Each boost pack purchased in the month extends it by BOOST_SPEND (70% of pack price).
export const PLANS = {
  basic:     { docs: 20,  refine: false, refineMonth: 0,   capTokens: 1_500_000, spendCap: 273 },
  pro:       { docs: 60,  refine: true,  refineMonth: 60,  capTokens: 4_000_000, spendCap: 1043 },
  director:  { docs: 120, refine: true,  refineMonth: 120, capTokens: 8_000_000, spendCap: 2443 },
  unlimited: { docs: Infinity, refine: true, refineMonth: Infinity, capTokens: Infinity, spendCap: Infinity },
};
export const PACK_PRICE = 490;  // ฿ per credit pack — server-side truth; the client renders this, never a hardcode
export const BOOST_SPEND = 343; // ฿ — 70% of the PACK_PRICE pack, keeps the 30% floor through top-ups

// ฿ per 1M tokens (in/out) per cost bucket — server-side truth for spend metering.
// galdr is rated at Gemini Flash (its Haiku fallback is rare and cheap).
export const RATES = {
  ...
  galdr:    { in: 15,  out: 90   },   // Gemini Flash-class
};
