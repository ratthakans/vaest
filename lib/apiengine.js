import { capFor, readUsage, readUsageData, writeUsageRow, checkDocQuota, applyDocBump, costTHB, applySpend, spendThisMonth, spendCapFor } from './plans.js';
...
export async function runApiEngine(req, res, opts) {
  ...
  if (plan && Number.isFinite(plan.spendCap)) {
    try {
      const d = await readUsageData(auth.email);
      if (spendThisMonth(d) >= spendCapFor(plan, d)) { res.status(429).json({ error: 'Monthly usage limit reached — add a credit pack or upgrade your plan' }); return null; }
