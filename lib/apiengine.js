...
export async function runApiEngine(req, res, opts) {
  ...
  if (opts.countsRefine) {
    try { const q = await checkRefineQuota(auth.email, plan, ud); if (!q.ok) { res.status(429).json({ error: q.planHasRefine ? 'This month’s Refine allowance is used up — it refreshes on the 1st. Add a usage credit pack, or upgrade.' : 'Refine unlocks on Pro — or add a usage credit pack to use it now.' }); return null; } }
