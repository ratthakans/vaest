...
export async function runApiEngine(req, res, opts) {
  ...
  try {
    ...
    if (opts.openai) {
      ...
      // specialty Anthropic models (e.g. Fable for present) fall back to the workhorse if unavailable
    ...
    // spend meter: rate by the engine that actually ran (fallbacks bill as what really answered)
    const ranModel = String(out.fellBack ? (opts.fallback || '') : (opts.openai || opts.model || ''));
